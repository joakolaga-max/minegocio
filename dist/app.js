...
// MiNegocio v2.0 - Built 2026-05-14T13:58:40.496Z
...
function useAppData() {
    ...
    const loadAll = (0, react_1.useCallback)(async () => {
        setSyncing(true);
        try {
            const [provData, misData, config, stockData, ventasData, fotosData, pedidosData, pedHistData] = await Promise.all(PATHS.map(p => (0, firebase_1.loadFromFirebase)(p)));
            setData(d => {
                const newData = {
                    ...d,
    ...
    (0, react_1.useEffect)(() => {
        ...
        const t = setTimeout(async () => {
            ...
            if (s('margenes') || s('misProductos'))
                saves.push((0, firebase_1.saveToFirebase)('config', { margenes: data.margenes }));
            if (s('stock'))
...
function TabCalculadora({ data, setData, showToast }) {
    ...
    const agregarProducto = (0, react_1.useCallback)((codigoRef) => {
        ...
        const pvTotal = (0, utils_1.calcPrecioVenta)(prod.precioCosto, prod.margen, data.margenes);
    ...
    const confirmarVenta = () => {
        ...
        if (!window.confirm(`Confirmar venta por ${(0, utils_1.fmtPeso)(total)}?`))
            return;
        setData(d => {
            // Discount stock
        ...
        react_1.default.createElement("div", { style: { position: 'relative', marginBottom: 12 } },
            react_1.default.createElement("div", { style: { display: 'flex', gap: 8 } },
                react_1.default.createElement("div", { style: { position: 'relative', flex: 1 } },
                    ...
                    react_1.default.createElement("input", { className: "input-field", style: { paddingLeft: 38 }, placeholder: "Buscar por REF, cod proveedor o descripci\u00F3n...", value: busqueda, onChange: e => { setBusqueda(e.target.value.toUpperCase()); setShowSuggestions(true); }, onKeyDown: e => { if (e.key === 'Enter')
                            agregarProducto(busqueda); if (e.key === 'Escape')
                            setShowSuggestions(false); }, onFocus: () => setShowSuggestions(true) })),
                react_1.default.createElement("button", { className: "btn-ghost", onClick: () => setScanning(true), style: { padding: '10px 14px' } },
                    react_1.default.createElement(Icon_1.Icon, { name: "camera", size: 20 }))),
            showSuggestions && sugerencias.length > 0 && (react_1.default.createElement("div", { style: {
                ...
                return (react_1.default.createElement("div", { key: i, onClick: () => agregarProducto(p.codigoRef), style: {
                    ...
                    foto && react_1.default.createElement("img", { src: foto, alt: "", style: { width: 36, height: 36, borderRadius: 6, objectFit: 'cover', flexShrink: 0 } }),
                    react_1.default.createElement("div", { style: { flex: 1, minWidth: 0 } },
                        react_1.default.createElement("div", { style: { display: 'flex', gap: 6, alignItems: 'center' } },
                            react_1.default.createElement("span", { style: { fontSize: 12, color: '#818cf8', fontFamily: 'monospace', fontWeight: 700 } }, p.codigoRef),
                    ...
                    react_1.default.createElement("div", { style: { fontSize: 13, color: '#22c55e', fontWeight: 700, flexShrink: 0 } },
                        (0, utils_1.fmtPesoInt)(pv / div),
                        div > 1 ? ' c/u' : '')));
            })))),
        items.length === 0 ? (react_1.default.createElement("div", { style: { textAlign: 'center', padding: '40px 20px', color: '#374151' } },
            react_1.default.createElement(Icon_1.Icon, { name: "store", size: 44 }),
            react_1.default.createElement("div", { style: { marginTop: 12, fontSize: 14, color: '#6b7280' } }, "Busc\u00E1 un producto para agregar"))) : (react_1.default.createElement(react_1.default.Fragment, null,
            react_1.default.createElement("div", { style: { display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 } }, items.map((item, i) => (react_1.default.createElement("div", { key: i, style: { background: '#111827', borderRadius: 12, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 10 } },
                react_1.default.createElement("div", { style: { flex: 1, minWidth: 0 } },
                    react_1.default.createElement("div", { style: { fontSize: 13, color: '#f1f5f9', fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, item.descripcion),
                ...
                react_1.default.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 6 } },
                    react_1.default.createElement("button", { onClick: () => setItems(its => its.map((x, j) => j === i ? { ...x, cantidad: Math.max(1, x.cantidad - 1) } : x)), style: { width: 28, height: 28, borderRadius: 6, background: '#374151', border: 'none', color: '#f1f5f9', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' } }, "\u2212"),
                    react_1.default.createElement("input", { type: "number", min: 1, value: item.cantidad, onChange: e => setItems(its => its.map((x, j) => j === i ? { ...x, cantidad: Math.max(1, parseInt(e.target.value) || 1) } : x)), style: { width: 44, height: 28, borderRadius: 6, background: '#1e2230', border: '1px solid #374151', color: '#f1f5f9', textAlign: 'center', fontSize: 13, fontWeight: 700, fontFamily: 'inherit' } }),
                    react_1.default.createElement("button", { onClick: () => setItems(its => its.map((x, j) => j === i ? { ...x, cantidad: x.cantidad + 1 } : x)), style: { width: 28, height: 28, borderRadius: 6, background: '#6366f1', border: 'none', color: '#fff', cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' } }, "+"),
                    react_1.default.createElement("div", { style: { width: 70, textAlign: 'right', fontSize: 13, color: '#22c55e', fontWeight: 700 } }, (0, utils_1.fmtPesoInt)(item.precioVenta * item.cantidad)),
                    react_1.default.createElement("button", { onClick: () => setItems(its => its.filter((_, j) => j !== i)), style: { background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.3)', color: '#ef4444', borderRadius: 6, padding: '5px 7px', cursor: 'pointer', display: 'flex', alignItems: 'center' } },
            ...
            react_1.default.createElement("div", { style: { display: 'flex', gap: 8 } },
                react_1.default.createElement("button", { className: "btn-ghost", style: { flex: 1, justifyContent: 'center' }, onClick: () => { if (window.confirm('Limpiar calculadora?'))
                        setItems([]); } }, "Limpiar"),
                react_1.default.createElement("button", { className: "btn-primary", style: { flex: 2, justifyContent: 'center' }, onClick: confirmarVenta },
                    react_1.default.createElement(Icon_1.Icon, { name: "check", size: 16 }),
                    " Confirmar venta")))),
        scanning && (react_1.default.createElement(Scanner_1.Scanner, { onResult: code => { setScanning(false); agregarProducto(code); }, onClose: () => setScanning(false) }))));
}
...
function App() {
    const { data, setData, loaded, syncing } = (0, useAppData_1.useAppData)();
    ...
    return (react_1.default.createElement("div", { style: { minHeight: '100vh', background: '#0d1117', color: '#f1f5f9', fontFamily: "'Space Grotesk', system-ui, sans-serif" } },
        react_1.default.createElement("div", { style: {
            ...
            react_1.default.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10 } },
                react_1.default.createElement("div", { style: {
                    ...
                    react_1.default.createElement(Icon_1.Icon, { name: "store", size: 18 })),
            ...
            react_1.default.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 10 } },
                react_1.default.createElement("div", { style: { display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 } },
                    react_1.default.createElement("div", { style: { width: 7, height: 7, borderRadius: '50%', background: syncing ? '#fbbf24' : '#22c55e' } }),
                    react_1.default.createElement("span", { style: { color: '#6b7280' } }, syncing ? 'Guardando...' : 'Sincronizado')),
        ...
        react_1.default.createElement("div", { style: { paddingTop: 60, paddingBottom: 80, maxWidth: 640, margin: '0 auto', padding: '76px 12px 80px' } }, !loaded ? (react_1.default.createElement("div", { style: { textAlign: 'center', padding: '80px 20px', color: '#6b7280' } },
            react_1.default.createElement("div", { style: { fontSize: 40, marginBottom: 16 } }, "\u26A1"),
            react_1.default.createElement("div", null, "Cargando..."))) : (react_1.default.createElement(react_1.default.Fragment, null,
            tab === 'calc' && react_1.default.createElement(TabCalculadora_1.TabCalculadora, { data: data, setData: setData, showToast: showToast }),
            tab !== 'calc' && (react_1.default.createElement("div", { className: "card", style: { textAlign: 'center', padding: '60px 20px', color: '#6b7280' } },
                react_1.default.createElement("div", { style: { fontSize: 40, marginBottom: 16 } }, "\uD83D\uDEA7"),
                react_1.default.createElement("div", { style: { fontSize: 16, fontWeight: 600, marginBottom: 8, color: '#f1f5f9' } }, "En construcci\u00F3n"),
                react_1.default.createElement("div", { style: { fontSize: 14 } }, "Esta pesta\u00F1a est\u00E1 siendo migrada")))))),
        react_1.default.createElement("div", { style: {
                position: 'fixed', bottom: 0, left: 0, right: 0,
                background: 'rgba(13,17,23,0.98)', borderTop: '1px solid #1e2535',
                display: 'flex', justifyContent: 'space-around', padding: '8px 0 12px',
            } },
            react_1.default.createElement("button", { onClick: () => switchTab('calc'), style: {
                    background: 'none', border: 'none', cursor: 'pointer', color: tab === 'calc' ? '#818cf8' : '#4b5563',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, fontSize: 10, fontFamily: 'inherit',
                } },
                react_1.default.createElement(Icon_1.Icon, { name: "store", size: 20 }),
                "Calculadora"),
            react_1.default.createElement("button", { onClick: () => setMenuOpen(v => !v), style: {
        ...
        menuOpen && (react_1.default.createElement(react_1.default.Fragment, null,
            ...
            react_1.default.createElement("div", { style: {
                    ...
                    zIndex: 200, padding: '8px 0 24px',
                } },
                react_1.default.createElement("div", { style: { width: 40, height: 4, background: '#374151', borderRadius: 2, margin: '8px auto 16px' } }),
                NAV_ITEMS.map(item => (react_1.default.createElement("button", { key: item.id, onClick: () => switchTab(item.id), style: {
                        display: 'flex', alignItems: 'center', gap: 14, width: '100%',
                        padding: '14px 20px', background: tab === item.id ? 'rgba(99,102,241,0.1)' : 'none',
                        border: 'none', cursor: 'pointer', color: tab === item.id ? '#818cf8' : '#94a3b8',
                    ...
                    item.label,
                    tab === item.id && react_1.default.createElement("div", { style: { marginLeft: 'auto', color: '#6366f1' } }, "\u2713"))))))),

...
const REACT_HOOKS = [
  ...
  'useReducer','useLayoutEffect','useId','createContext','createRef',
...
for (const file of files) {
  ...
  code = code.replace(/var __importDefault\s*=[\s\S]*?return.*?;\s*\};\s*/g, '');
  code = code.replace(/var __importStar\s*=[\s\S]*?return.*?;\s*\};\s*/g, '');

  // Remove react/reactdom require lines
  ...
  // Replace remaining require() -> __require()
...
// Fix default export components (App_1.default etc)
bundle = bundle.replace(/\b(\w+)_1\.default\b/g, (_, name) => {
  const key = name.toLowerCase();
  return `(__modules['${name}'] && (__modules['${name}'].default || __modules['${name}']))`;
});

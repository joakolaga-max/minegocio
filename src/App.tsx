...
export default function App() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117', color: '#f1f5f9', fontFamily: "'Space Grotesk', system-ui, sans-serif" }}>
      ...
      {/* Content */}
      <div style={{ paddingTop: 60, paddingBottom: 80, maxWidth: 640, margin: '0 auto', padding: '76px 12px 80px' }}>
        {!loaded ? (
          ...
          <>
            {tab === 'calc' && <TabCalculadora data={data} setData={setData} showToast={showToast} />}
            {tab !== 'calc' && (
              <div className="card" style={{ textAlign: 'center', padding: '60px 20px', color: '#6b7280' }}>
                <div style={{ fontSize: 40, marginBottom: 16 }}>🚧</div>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8, color: '#f1f5f9' }}>En construcción</div>
                <div style={{ fontSize: 14 }}>Esta pestaña está siendo migrada</div>
              </div>
            )}
          </>
        )}
      ...
      {menuOpen && (
        <>
          ...
          <div style={{
            ...
            <div style={{ width: 40, height: 4, background: '#374151', borderRadius: 2, margin: '8px auto 16px' }} />
            {NAV_ITEMS.map(item => (
              <button
                key={item.id}
              ...
              >
                ...
                {item.label}
                {tab === item.id && <div style={{ marginLeft: 'auto', color: '#6366f1' }}>✓</div>}
              </button>
            ))}
          </div>
        </>

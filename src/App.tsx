...
export default function App() {
  ...
  if (!user) {
    return <LoginScreen onLogin={() => setUser((window as any).__user)} />;
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0d1117', color: '#f1f5f9', fontFamily: "'Space Grotesk', system-ui, sans-serif" }}>
      ...
      {/* Content */}
      <div style={{ paddingTop: 60, maxWidth: 640, margin: '0 auto', padding: '76px 12px 80px' }}>
        {!loaded ? (
          ...
          <>
            ...
            {tab !== 'calc' && (
              <div style={{ background: '#161b27', borderRadius: 16, border: '1px solid #1e2535', padding: '60px 20px', textAlign: 'center', color: '#6b7280' }}>
                <div style={{ fontSize: 40, marginBottom: 16 }}>🚧</div>
                <div style={{ fontSize: 16, fontWeight: 600, marginBottom: 8, color: '#f1f5f9' }}>En construcción</div>
                <div style={{ fontSize: 14 }}>Esta pestaña se está migrando</div>
              </div>
            )}
          </>
        )}
      ...
      <div style={{
        ...
        <button onClick={() => switchTab('calc')} style={{
